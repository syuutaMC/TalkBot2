...
**新しいコードを追加・変更するとき、必ず関連するコードも合わせて更新してください。**

#### ギルド固有データを追加・削除するとき

| 変更内容 | 合わせて更新すべき箇所 |
|----------|----------------------|
| `bot.guild_configs` / `bot.voice_queues` / `bot.is_playing` などギルドごとのデータ構造を追加 | `on_guild_remove` でそのデータを `.pop(guild_id, None)` で削除する処理を追加 |
| ギルドごとのデータを追加 | `on_guild_join` で初期値を設定する処理を追加（必要な場合） |
| ギルドごとのデータを追加 | `on_ready` でBotが既に参加しているギルドに対して初期化する処理を追加（必要な場合） |

#### 永続化データ（config）を追加・変更するとき

| 変更内容 | 合わせて更新すべき箇所 |
|----------|----------------------|
| 新しいデータを `config.json` に保存する場合 | `_save_config()` の `config` 辞書に項目を追加 |
| 新しいデータを `config.json` から読み込む場合 | `_load_config()` でその項目をパースして `self.〇〇` に代入する処理を追加 |
| config のキー型が変わる場合 | `_load_config()` 内のキー変換処理も合わせて修正 |

#### イベントハンドラを追加・変更するとき

| 変更内容 | 合わせて更新すべき箇所 |
|----------|----------------------|
| 新しいイベントハンドラを追加 | `tests/test_bot.py` に対応するテストクラス・メソッドを追加 |
| `on_guild_remove` に新しいクリーンアップ処理を追加 | `test_on_guild_remove_*` テストに対応するアサーションを追加 |
| `on_guild_join` を変更 | `test_on_guild_join_*` テストを更新 |

#### コマンドを追加・変更するとき

| 変更内容 | 合わせて更新すべき箇所 |
|----------|----------------------|
| 新しいスラッシュコマンドを追加 | `tests/test_bot.py` にそのコマンドのテストを追加 |
| コマンドの引数・動作を変更 | 既存テストを更新し、README.md のコマンド一覧も更新 |
| コマンドを削除 | 対応するテストも削除し、README.md を更新 |

#### 新しいモジュール（`src/〇〇.py`）を追加するとき

| 変更内容 | 合わせて更新すべき箇所 |
|----------|----------------------|
| `src/` に新しいファイルを追加 | `tests/test_〇〇.py` を同時に作成 |
| `src/` に新しいファイルを追加 | このファイル（`copilot-instructions.md`）の「プロジェクト構造」に追記 |
| 新しいモジュールが新技術を使う場合 | `.github/skills/` に対応するスキルを追加 |
...
3. **README.md の定期更新**:
   - コマンドの追加・変更・削除 → 「コマンド一覧」テーブルを更新
   - 環境変数の追加・変更 → 「環境変数」テーブルを更新
   - プロジェクト構造の変更（ファイル追加・削除） → 「プロジェクト構造」ツリーを更新
   - Docker構成の変更 → 「起動方法」セクションを更新
   - 新機能の追加 → 「使用方法」セクションに使い方を追記
   - アーキテクチャの変更 → Mermaid図を更新
   - **documentationスキルのガイドラインに従うこと**
...
- **commit**: コミットメッセージのガイドライン
  - 明確で原子的なコミットメッセージの作成
- **documentation**: ドキュメント作成・更新のベストプラクティス
  - README、API仕様書、セットアップガイド、トラブルシューティングの記述ガイド
  - コマンド一覧、環境変数、アーキテクチャ図の作成方法
...
@documentation README.mdにこの機能の説明を追加してください
```
